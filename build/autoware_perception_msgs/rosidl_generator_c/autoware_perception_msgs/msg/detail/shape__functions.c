  ...
  autoware_perception_msgs__msg__Shape__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    autoware_perception_msgs__msg__Shape * data =
      (autoware_perception_msgs__msg__Shape *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!autoware_perception_msgs__msg__Shape__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          autoware_perception_msgs__msg__Shape__fini(&data[i]);
        }
        free(data);
        return false;
      }
