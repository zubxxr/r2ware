  ...
  autoware_planning_msgs__msg__Path__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    autoware_planning_msgs__msg__Path * data =
      (autoware_planning_msgs__msg__Path *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!autoware_planning_msgs__msg__Path__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          autoware_planning_msgs__msg__Path__fini(&data[i]);
        }
        free(data);
        return false;
      }
