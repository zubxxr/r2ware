  ...
  autoware_system_msgs__msg__HazardStatus__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    autoware_system_msgs__msg__HazardStatus * data =
      (autoware_system_msgs__msg__HazardStatus *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!autoware_system_msgs__msg__HazardStatus__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          autoware_system_msgs__msg__HazardStatus__fini(&data[i]);
        }
        free(data);
        return false;
      }
