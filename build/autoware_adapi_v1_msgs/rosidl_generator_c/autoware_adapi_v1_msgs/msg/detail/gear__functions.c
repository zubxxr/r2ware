  ...
  autoware_adapi_v1_msgs__msg__Gear__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    autoware_adapi_v1_msgs__msg__Gear * data =
      (autoware_adapi_v1_msgs__msg__Gear *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!autoware_adapi_v1_msgs__msg__Gear__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          autoware_adapi_v1_msgs__msg__Gear__fini(&data[i]);
        }
        free(data);
        return false;
      }
