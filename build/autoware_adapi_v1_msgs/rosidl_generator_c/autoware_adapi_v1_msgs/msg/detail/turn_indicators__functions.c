  ...
  autoware_adapi_v1_msgs__msg__TurnIndicators__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    autoware_adapi_v1_msgs__msg__TurnIndicators * data =
      (autoware_adapi_v1_msgs__msg__TurnIndicators *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!autoware_adapi_v1_msgs__msg__TurnIndicators__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          autoware_adapi_v1_msgs__msg__TurnIndicators__fini(&data[i]);
        }
        free(data);
        return false;
      }
