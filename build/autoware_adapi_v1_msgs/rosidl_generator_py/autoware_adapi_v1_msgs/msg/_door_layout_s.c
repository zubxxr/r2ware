...
bool autoware_adapi_v1_msgs__msg__door_layout__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("autoware_adapi_v1_msgs.msg._door_layout.DoorLayout", full_classname_dest, 50) == 0);
  }
  autoware_adapi_v1_msgs__msg__DoorLayout * ros_message = _ros_message;
  {  // roles
    PyObject * field = PyObject_GetAttrString(_pymsg, "roles");
    if (!field) {
      return false;
    }
    if (PyObject_CheckBuffer(field)) {
      ...
      uint8_t * dest = ros_message->roles.data;
      ...
      PyObject * seq_field = PySequence_Fast(field, "expected a sequence in 'roles'");
      if (!seq_field) {
        Py_DECREF(field);
        return false;
      }
...
{
  /* NOTE(esteve): Call constructor of DoorLayout */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("autoware_adapi_v1_msgs.msg._door_layout");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "DoorLayout");
  ...
  {  // roles
    PyObject * field = NULL;
    field = PyObject_GetAttrString(_pymessage, "roles");
    if (!field) {
      return NULL;
    }
    ...
    if (ros_message->roles.size > 0) {
      // populating the array.array using the frombytes method
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      uint8_t * src = &(ros_message->roles.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->roles.size * sizeof(uint8_t));
      assert(data != NULL);
