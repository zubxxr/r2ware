  ...
  autoware_system_msgs__msg__AutowareState__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    autoware_system_msgs__msg__AutowareState * data =
      (autoware_system_msgs__msg__AutowareState *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!autoware_system_msgs__msg__AutowareState__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          autoware_system_msgs__msg__AutowareState__fini(&data[i]);
        }
        free(data);
        return false;
      }
