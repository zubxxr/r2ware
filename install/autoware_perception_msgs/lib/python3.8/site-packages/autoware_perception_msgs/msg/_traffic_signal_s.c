...
bool autoware_perception_msgs__msg__traffic_signal__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // elements
    ...
    if (!autoware_perception_msgs__msg__TrafficSignalElement__Sequence__init(&(ros_message->elements), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create autoware_perception_msgs__msg__TrafficSignalElement__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    autoware_perception_msgs__msg__TrafficSignalElement * dest = ros_message->elements.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!autoware_perception_msgs__msg__traffic_signal_element__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
PyObject * autoware_perception_msgs__msg__traffic_signal__convert_to_py(void * raw_ros_message)
{
  ...
  {  // elements
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      PyObject * pyitem = autoware_perception_msgs__msg__traffic_signal_element__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
